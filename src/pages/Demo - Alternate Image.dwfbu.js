...
function repeater_onItemReady($item, data) {
    // Set the mouse in/out triggers to alternate the source - loads the source each time, takes longer
    $item('#img').onMouseIn(() => $item('#img').src = data.altImage)
    $item('#img').onMouseOut(() => $item('#img').src = data.mainImage)

    // Set the mount in/out triggers to hide/show the alternate image elements - the pictures are both loaded, instant
